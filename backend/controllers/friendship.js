...
exports.sendFriendReq = async(req, res) => {
    try{
        ...
        const newRequest = await Friendship.create({user1: userId, user2: friendId, movieId: movieId});
        if(newRequest){
    ...
    try {
        ...
        if (!user || !user.friends || user.friends.length === 0) {
            return res.status(200).json({
                success: true,
                data: [],
                message: "No friends found for this user or all friendship requests are pending.",
            });
        }

        const friends = await Promise.all(user.friends.map(async (friendship) => {
            const friendId = friendship.user1.toString() !== userId ? friendship.user1 : friendship.user2;
...
exports.getPendingRequests = async(req, res) => {
    try{
        ...
        await Promise.all(pendingRequests.map(async (friend) => {
            const friendUser = friend.user1.toString() === userId ? friend.user2 : friend.user1;
